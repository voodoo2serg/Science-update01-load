...
def create_program_requirement(program_id: str, payload: schemas.ProgramRequirementCreate, db: Session = Depends(get_db)):
    program = service_layer.get_program(db, program_id)
    if not program:
        raise HTTPException(status_code=404, detail="Program not found")
    requirement = service_layer.create_program_requirement(db, program_id, payload)

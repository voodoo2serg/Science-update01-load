...
from app.models import Payment, User, SubscriptionPlan
...
def activate_demo_subscription(user_id: str, plan_name: str, db: Session = Depends(get_db)):
    ...
    payment = Payment(user_id=user.id, amount=plan.price_rub, status='succeeded', transaction_id='demo')
    db.add(payment)
    db.commit()

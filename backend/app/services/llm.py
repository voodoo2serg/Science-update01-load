...
from app.services.policy import PolicyVerdict, classify_request
...
def generate_safe_draft(topic: str, title: str, draft_type: str) -> str:
    verdict = classify_request(f'{title}\n{topic}\n{draft_type}')
    if verdict == PolicyVerdict.PROHIBITED:
        raise PolicyError(
            'Запрос заблокирован политикой безопасности. Сервис помогает со структурой, логикой и планом, '
            'но не пишет готовые диссертации, статьи и не помогает обходить антиплагиат.'
        )
